...
function MesMatchScreens({navigation,theToken,reduxFunction,addFavStore,majFavStore,theFavList}) {
  ...
  // }
   ...
   var addLike = async (data)=>{
    var envoiAnnonce = await fetch('http://192.168.1.24:3000/addLike',{
       method: 'POST',
...
useEffect(() => {
  var recupBdd = async() =>{
    var sendToken  = await fetch('http://192.168.1.24:3000/saveToStore',{
      method: 'POST',
      headers: {'Content-Type':'application/x-www-form-urlencoded'},
     body:`token=${theToken}`
    })
  ...
  var  fetchData= async ()=> {
    var rawResponse =  await fetch("http://192.168.1.24:3000/mesMatchs",{
      method: 'POST',
      headers: {'Content-Type':'application/x-www-form-urlencoded'},
      body: `token=${theToken}`
    })
